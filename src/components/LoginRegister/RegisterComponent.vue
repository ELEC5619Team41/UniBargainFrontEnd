  ...
  <div>
    ...
    <div style="display: flex">
      <div>
        ...
        <LabelTextField class="regiEmail" :text="$t('EmailAddress')" v-model:inputText="this.form.email"></LabelTextField>
        <div class="EmailVeri"><RoundCornerButton  :text="'verify email'" @click="emailVeri"></RoundCornerButton></div>
  ...
  methods: {
    ...
    requestRegister() {
      if(this.form.password != this.form.confirmPassword){
        alert("The two inputs of password is different, please try again!");
      }else if(!this.form.agreement ){
    ...
    async SendRegisterRequest(){
      ...
      if(!this.emailv){
        alert("email is not valid or haven't been verified.")
      }else{
      ...
      fetch("http://localhost:28888/user/register", requestOptions)
        .then(response => response.json())
        .then(data => {
          alert("Register successfully! Jumping to login page");
          this.$router.go();
        })
        .catch(error => console.log('error', error));
    }
